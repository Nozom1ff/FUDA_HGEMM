...
// Overload for kernel with M,N,K parameters
template <typename T>
float perf_gemm(
...
float gemm_error_check_nn(
    ...
    // For row-major matrices, we need to compute C^T = B^T * A^T
    // This is equivalent to C = A * B in row-major
    cublasGemmEx(handle,
                 CUBLAS_OP_N,
                 CUBLAS_OP_N,
                 N, M, K,
    ...
    // Direct comparison - cuBLAS produces the same layout as our kernel
    float max_error = 0.0;
    for (int i = 0; i < M * N; i++) {
        float this_error = fabsf((float)h_c_ref[i] - (float)h_c[i]);
...
// Overload for kernel with const T*, const T*, T* signature
template <typename T>
float gemm_error_check_nn(
    void (*gpu_hgemm) (const T *, const T *, T *, int, int, int),
    int M, int N, int K) {
...
// Calculate TFLOPS
inline float calculate_tflops(int M, int N, int K, float time_sec) {
    // 2 * M * N * K flops for matrix multiply
    float gflops = (2.0f * M * N * K) / (time_sec * 1e9f);
    return gflops / 1000.0f; // Convert to TFLOPS
}
